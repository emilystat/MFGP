function [K_em, gamma_em, tau2_em, alpha_em, T, epsilon, time] = EM_MCAR_2Vars(S, ...
			A, num_vars, r, n, z, H, sig2eps, Delta, maxit, avgtol, opts)       
%% input:
% S: is an n x r basis function matrix.
...
%[gamma; tau2; alpha]
lb = [lbg; lbg; lbt; lbt; -10; -10];
ub = [ubg; ubg; ubt; ubt; 10 ; 10];
...
P = alpha(1,t) * speye(N) + alpha(2,t) * H;

M = [ Q1+P'*Q2*P, -P'*Q2; -Q2*P, Q2 ];
...
function f = neg2Q(theta)   
    ...
    f_Q1 = (DeltaInv-theta(1)*H)/theta(3);
    f_Q2 = (DeltaInv-theta(2)*H)/theta(4);
    f_P = theta(5) * speye(N) + theta(6) * H;

    f_M = [ f_Q1+f_P'*f_Q2*f_P, -f_P'*f_Q2; -f_Q2*f_P, f_Q2 ];
    Tmid = f_M + A2A; 
